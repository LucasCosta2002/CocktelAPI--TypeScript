...
export default function Modal() {
    ...
    const renderIngridients = () => {
        ...
        for (let i = 1; i <= 6; i++) {
            //le digo que ese ingrediente pertence al type RecipeDrink
            const ingridient = selectedRecipe[`strIngredient${i}` as keyof RecipeDrink];    
            const measure = selectedRecipe[`strMeasure${i}` as keyof RecipeDrink];    
                            ...
                            >
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6" >
                                    ...
                                    <div className='mt-5 flex justify-between gap-4'>
                                        ...
                                        >{favoriteExists(selectedRecipe.idDrink) ? "Quitar de Favoritos" : "Agregar a Favoritos"}</button>
                                    </div>
